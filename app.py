...
def show_challenges():
    ...
    if not user:
        user = User(name=user_name, points=0)
        db.session.add(user)
        db.session.commit()
...
with app.app_context():
    if Challenge.query.count() == 0:
        db.session.add_all([
            Challenge(title="Plant a Tree", description="Plant 1 tree", points_reward=10),
            Challenge(title="Recycle Waste", description="Segregate waste", points_reward=15),
